...
群聊邀请，除了被邀请人会收到邀请通知，群内同时也会推送卡片消息，卡片消息数据结构如下，卡片消息 contentType 为 1611
...
| ---------- | ------------------------------------------------- | ------------- |
| channelID  | string                                            | 频道 id       |
| opUser     | [PublicInfo](docs/sdks/class/user/publicInfo.mdx) | 用户数组      |
...
```
如果是单聊，同时会推送拒绝消息，contentType为1612
...
```
如果是群聊，同时群内会推送用户加入语音房消息，contentType为1613
...
```
如果是单聊，同时会推送取消消息，contentType为1614
...
```
如果是单聊，同时会推送关闭消息，contentType为1615
