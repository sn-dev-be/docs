...
:::info 说明
当加入需审核的部落时，拥有管理部落成员权限的用户将会收到消息，消息发送人 id 为 clubAdmin
:::

### 加入部落通知

用户申请加入部落，将会收到 contentType 为 1803 的消息，消息负载数据结构如下
...
管理员通过申请，部落内所有管理员都会收到 contentType 为 1805 的消息，消息负载数据结构如下

| 名称       | 类型                                              | 描述                 |
| ---------- | ------------------------------------------------- | -------------------- |
| server     | [ServerInfo](docs/sdks/class/club/serverInfo.mdx) | 部落信息             |
| opUser     | [PublicInfo](docs/sdks/class/user/publicInfo.mdx) | 申请人信息           |
| handleMsg  | string                                            | 处理信息             |
| receiverAs | int32                                             | 1 admin/ 0 applicant |

### 加入部落拒绝通知

管理员拒绝申请，部落内所有管理员都会收到 contentType 为 1806 的消息，消息负载数据结构如下
