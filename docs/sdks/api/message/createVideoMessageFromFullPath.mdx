...
创建视频消息，由 SDK 内部根据全路径提取视频信息并创建视频消息。
...
| placeholderSnapshot | NSString  | 是       | base64 编码视频封面占位图 |
...
```dart showLineNumbers
    public Message createVideoMessageFromFullPath(String videoPath, String videoType, long duration, String snapshotPath, String placeholderSnapshot)
```
...
| duration            | int      | 是       | 时长                      |
| snapshotPath        | String   | 是       | 视频封面路径              |
| placeholderSnapshot | String   | 是       | base64 编码视频封面占位图 |
...
Web 端建议使用[createVideoMessageByURL](docs/sdks/api/message/createVideoMessageByURL.mdx)或[createVideoMessageByFile](docs/sdks/api/message/createVideoMessageByFile.mdx)。
