...
创建图片消息，由 SDK 内部根据全路径提取图片信息并创建图片消息。
...
| encodePlaceholderImage | NSString | 是       | base64 编码占位图 |
...
```dart showLineNumbers
   public Message createImageMessageFromFullPath(String imagePath, String encodePlaceholderImage)
```
...
| ---------------------- | -------- | -------- | ----------------- |
| imagePath              | String   | 是       | 绝对路径          |
| encodePlaceholderImage | String   | 是       | base64 编码占位图 |
...
```dart showLineNumbers
Message Message= OpenIMClient.getInstance().messageManager. createImageMessageFromFullPath( imagePath, encodePlaceholderImage);
    //todo
...
Web 端建议使用[createImageMessageByURL](docs/sdks/api/message/createImageMessageByURL.mdx)或[createImageMessageByFile](docs/sdks/api/message/createImageMessageByFile.mdx)。
